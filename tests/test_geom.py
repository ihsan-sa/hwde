...
# A hand-written 4-layer board covering every parser path: segment, through via,
# rotated SMD roundrect pad, *.Cu thru-hole pad, a filled zone and an UNFILLED
# zone, and a gr_rect outline. Coordinates chosen so areas are analytic.
SYN4 = """
(kicad_pcb
  (version 20260206)
  (generator "test")
...
def test_counts(syn):
    assert len(syn.tracks_of()) == 1
    assert len(syn.vias_of()) == 1
    assert len(syn.pads_of()) == 2
    assert len(syn.zones_of()) == 2
...
def test_stackup_from_board_block(tmp_path):
    # An explicit (stackup) block -> parsed heights/epsilon, assumed=False.
    stackup = """    (stackup
