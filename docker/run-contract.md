...
You are the `/ai-ee` orchestrator (read `.claude/skills/ai-ee/SKILL.md` and follow
...
- KiCad 10.0.5: `kicad-cli` on PATH (`AIEE_KICAD_CLI=/usr/bin/kicad-cli`); the
...
- Java 25 + Freerouting 2.2.4 + KiCadRoutingTools 0.19.0 + libngspice + pdflatex
  are installed and pinned through `AIEE_*` env vars; `tools/` is empty by design.
