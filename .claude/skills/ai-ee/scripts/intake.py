#!/usr/bin/env python
"""intake.py - import an EXTERNAL KiCad project into an ai-ee workspace (T9).

The "review this board" front door: point it at any KiCad project (a directory,
a .kicad_pro/.kicad_pcb/.kicad_sch) and it produces a normal ai-ee workspace -
...
def plan_copy(spec: dict, board: str, kicad_share: Path | None = None,
    ...
    for table in ("fp-lib-table", "sym-lib-table"):
        ...
        for entry in lib_table_entries(tpath):
            ...
            libs.append(rec)
    # 4. ai-ee sidecars, when the source IS an ai-ee-shaped project (another
