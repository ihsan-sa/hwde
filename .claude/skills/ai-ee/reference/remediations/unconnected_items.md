...
## Verify
```
.venv\Scripts\python.exe .claude\skills\ai-ee\scripts\kc.py drc boards\<board>\kicad\<board>.kicad_pcb --refill --save-board --parity --all-track-errors --out boards\<board>\work\drc-after.json
.venv\Scripts\python.exe .claude\skills\ai-ee\scripts\gate.py --gate drc_routed boards\<board>\kicad\<board>.kicad_pcb
```
