...
```
cd C:\dev\ai-ee3
.venv\Scripts\python .claude\skills\ai-ee\scripts\check_return_path.py --pcb <ws>\<board>.kicad_pcb --constraints <ws>\constraints.json --verify-fill --out <ws>\reports\return_path.json
.venv\Scripts\python .claude\skills\ai-ee\scripts\gate.py --gate verify <ws>\<board>.kicad_pcb
```
