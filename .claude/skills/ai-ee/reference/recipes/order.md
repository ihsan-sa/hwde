...
2. `order_quote.py` - the quote matrix. `estimated: true` on every figure:
   ...
   the only real quote.
3. `order_submit.py` - assembles the manifest, the fab package paths, and the
   `human_steps` list. With `AIEE_JLCPCB_*` credentials, `--api` adds upload +
   the API's own DFM audit + a real `calculate` (a `scope_pending` response is
   normal).
4. **H5, always blocking.** Present: the quote row, the API audit findings beside
...
normalized design hash, so a changed design refuses rather than ordering the
wrong board. Payment itself is never automated.
