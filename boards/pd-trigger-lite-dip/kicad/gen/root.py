"""Generator for the pd-trigger-lite-dip schematic - ONE FLAT root sheet.

Source of truth for kicad/pd-trigger-lite-dip.kicad_sch (+ .kicad_pro,
decoupling.json). Rebuild:

    .venv/bin/python boards/pd-trigger-lite-dip/kicad/gen/root.py
...
Wiring facts (parts/C42459160.json, CH224 manual V2.1 tables 4-1/5-1, 6.1.1):
  ...
  2/3 CFG2/CFG3 float (internal pull-ups), 4/5 DP/DM unused (6-pin
  receptacle has no data contacts), 10 PG unused, 11 = GND pad.
  Rset table 5-1: 6.8k 9 V, 24k 12 V, 56k 15 V, 120k 20 V.
  I/O table 5-2: CFG1 = 1 -> 5 V (CFG2/3 don't care); 6.1.2: CFG1 high =
  100k to VHV. CFG1 abs max 3.8 V on CH224A (7.1).
SW1 1.27-5P TPPT (C7421518), 5x SPST: switch k joins pin k (row 1-5) to
  pin 11-k (row 6-10). Row 6-10 = CFG1 (common); pin k = SELx to its
  resistor. One-hot: pos1 5 V (100k to VHV), pos2-5 9/12/15/20 V (Rset).
...
LCSC = {
    ...
    "R5": "C25803", "R6": "C25804", "D1": "C2286", "R7": "C21189",
    "SW1": "C7421518",
}


def build() -> schlib.Sheet:
    sh = schlib.Sheet("pd-trigger-lite-dip",
                      title="pd-trigger-lite-dip: USB-C PD trigger, 5/9/12/15/20 V by DIP switch, 3 A",
    ...
    # ---- B3 voltage select: SW1 one-hot. Pos1 = CFG1 high via 100k to VHV
    # (I/O mode, 5 V, manual 5.2.2 / 6.1.2); pos2-5 = Rset to GND (5.2.1).
    sh.add_component("aiee:1.27-5PTPPT", "SW1", "DIP 5P 5/9/12/15/20V",
                     at=(203.2, 83.82),
                     footprint=f"{FP}:SW-SMD_5P-P1.27_1.27-5P")
    rows = [("R5", "aiee:0603WAF1003T5E", "100k", "SEL5", "VHV", "1"),
            ("R1", "aiee:0603WAF6801T5E", "6.8k", "SEL9", "GND", "2"),
            ("R2", "aiee:0603WAF2402T5E", "24k", "SEL12", "GND", "3"),
            ("R3", "aiee:0603WAF5602T5E", "56k", "SEL15", "GND", "4"),
            ("R4", "aiee:0603WAF1203T5E", "120k", "SEL20", "GND", "5")]
    sw = {}
    for i, (r, lib, val, mid, far, pos) in enumerate(rows):
        y = 55.88 + i * 12.7
        sh.add_component(lib, r, val, at=(165.1, y), footprint=R0603)
        sh.wire_pins(r, {"1": far, "2": mid})
        sw[pos] = mid                          # row 1-5: the selected leg
        sw[str(11 - int(pos))] = "CFG1"        # row 6-10: common CFG1
    sh.wire_pins("SW1", sw)
...
def main(argv=None) -> int:
    ...
    try:
        ...
        print(json.dumps({"script": "gen.pd-trigger-lite-dip", "status": "error",
                          "error": f"{type(exc).__name__}: {exc}"}))
        return 2
    print(json.dumps({"script": "gen.pd-trigger-lite-dip", "status": "pass",
                      "files": [str(sch), str(meta)],
                      "components": len(LCSC) + 1}, indent=1))
    return 0
