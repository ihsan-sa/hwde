...
pushed toward the side marked ON on the switch body. The board's silkscreen shows that side as
``\textless ON'' (toward the voltage labels), but the mark printed on the switch itself is the one
to trust. Use a toothpick or a small screwdriver; the sliders are 1.27 mm apart.
...
corner, next to R5, so switch 1 is the 5 V row; if JLC's model shows the ON side, it should face
the voltage labels. D1: its cathode mark must face the right board edge. U1: the pin-1 dot sits at
