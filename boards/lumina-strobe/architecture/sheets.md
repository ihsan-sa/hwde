...
| `/ID_ADC` | conn (local) | Board-type ID, **bottom leg only** - the carrier fits the 10 k top leg. **[REV E] `R_ID` = 2.7 kohm 1 % = ICD rev A6 s3.4 code 1 (LUM-STR-A), `V_ID` 0.702 V. ALLOCATED, not a placeholder** |
...
| `R2` | **[REV E] `R_ID` = 2.7 kohm 0603 1 %** | **ICD rev A6 s3.4 code 1 = LUM-STR-A**, `V_ID` 0.702 V. **NO LONGER A PLACEHOLDER.** The 4.7 kohm carried since rev A was **code 2 = LUM-PAR-A** - left alone this board would have announced itself as the par and been handed the wrong daughter profile |
...
| **`R102`, `R109`, `D101`, `C100`** | **[REV E] Floating rail: 2 x 8.2 k 0805 IN SERIES (= 16.4 k) + 12 V zener + 1 uF** | **The rev C single 24 k was self-conflicting** - it left as little as 0.30 mA to bias the zener against the LM2904B's 1.2 mA max Iq, *and* dissipated 84 mW at 57 V = 67 % of an 0805. 16.4 k split in two gives **1.00 mA of zener bias at the 48 V minimum** and **61.7 mW per part = 49 %**. Full arithmetic in `power_tree.md` s3.3.1. Dies with `+48V_SW`, which is correct - there is nothing to charge without it |
...
| **`R<N+4>`** | **[REV D] Gate series `R_g` = 100 ohm** | **Isolates the op-amp output from the FET's ~2000 pF `Ciss`, which is 20x the LM2904B's rated `CLOAD`.** Puts the gate pole at 796 kHz, 79x above the loop crossover. `blocks.md` s2.3.1 |
| **`R<N+8>`, `C<N+2>`** | **[REV D] Loop compensation: `R_in` = 10 k (shunt into the inverting input), `C_f` = 1 nF (output to inverting input)** | **Dominant-pole compensation - the capacitive load is INSIDE this loop by construction and cannot be isolated out of it.** Crossover 10.1 kHz, 1 % settling 73 us, total optical edge 85 us = **12x margin on STR-REQ-11** and 3.2 % of the shortest 2.67 ms pulse |
...
| **`R442`-`R449`, `R450`-`R453`** | **[REV D] HYSTERESIS NETWORKS - the LM2901 has none and its datasheet offers no guidance.** Per section: `R_a`/`R_b` reference divider + `R_hyst` from the open-collector output back to the reference node + `R_pull` **10 k to `+12V`** | **U400A board OT** 56 k / 10 k / **560 k** = 175 mV = **5.0 C** of tab; **U400B LED OT** 10 k / 10 k / **150 k** = 379 mV = **5.2 C** of solder point; **U400C UVLO** 22 k / 5.1 k / **820 k** = 59 mV = **1.02 V** of bank; **U400D ceiling** 20 k / 5.6 k / **620 k** = 82 mV = **1.42 V** of bank. Full arithmetic and the `Vio` judgement call in `blocks.md` s2.4.2. **The pull-up participates in the divider - it is not a free choice** |
| **`R454`-`R457`** | **[REV D] Vds hysteresis, ~1 M per section** | ~5 % of the trip threshold. **Non-critical because these four are latched** (`Q400`-`Q403`); the band exists only to hand each latch one clean edge |
| **`Q404`, `R458`, `R459`** | **[REV D] `/OT_TRIP` -> `FAULT` open-drain translator, 2N7002** | **New, and forced by the hysteresis:** the pull-ups put `/OT_TRIP` on a **`+12V`** node, which the four 2N7002 gate clamps want anyway, but a 12 V node cannot land on the carrier's 3.3 V `FAULT` pin. `Q404` translates it down as **open drain with no pull-up**, so the carrier's 10 k still owns the net and **`FAULT` is never driven high**. `blocks.md` s2.4.4 |
| **`D400`, `D401`, `R460`, `R461`, `C414`, `C415`** | **[REV D] `J300` ESD protection: bidirectional TVS (SMAJ12CA class) + 10 nF to GND per thermistor line, plus a 1 k series resistor into each comparator pin** | **The LM2901 is only 500 V HBM and `U400B`'s input reaches an off-board connector.** **The 1 k must sit AFTER the divider tap, not in the loom** - in the loom it joins the NTC leg and shifts the trip point by ~1 k against a ~1 k bottom leg. After the tap it carries 25 nA of bias = 25 uV |
...
| `C410`-`C413`, **`C416`** | Decoupling: **100 nF X7R 0603 at each of `U400`/`U401` `V+`, `U410`, `U420`, plus one shared 1 uF** | **[REV D] The LM2901 publishes no decoupling guidance and no layout section at all**, so this is a design decision `blocks.md` s2.4.3 owns explicitly - "not grounded", not "not needed". Do not leave it to a schematic agent |
