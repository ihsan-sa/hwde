...
| 160 | 2011 | drc_routed 0/0 does NOT imply fabricable | [gates][dfm] | L2 | L3 | scripts/rules_gen.py | open | T1 fixed the .kicad_pro side (fabfloors + ERROR severities, asserted). STILL UNOWNED and unchanged: nothing asserts that a HAND-WRITTEN .kicad_dru still contains rules_gen's aiee_* fab floors - the lumina-carrier failure path |
...
| 169 | 2141 | JLC's impedance-template list CHURNS - and the endpoint lies quietly w | [stackup][jlcapi][ordering] | L1 | L2 | reference/stackups.yaml | open | Provenance + live_verified dates are recorded per entry (L1) but nothing RE-PROBES: a template withdrawn next week is only caught by hand. L2 = a scripted freshness check on the template code at P5 for impedance-controlled boards (V18) |
